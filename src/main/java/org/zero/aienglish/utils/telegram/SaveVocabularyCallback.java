...
public class SaveVocabularyCallback implements TelegramCallback {
    ...
    public String getName() {
        return CallbackEnum.ADD_VOCABULARY.name();
    }
    ...
    public List<BotApiMethod<?>> apply(Update update) {
        ...
        if (callbackSplited.length < 1) {
            log.warn(
                    "Error occurred while saving word in chat -> {}, with callback -> {}",
                    chatId, update.getCallbackQuery().getData()
            );
        }
        var wordForSaveId = Integer.parseInt(callbackSplited[1]);
        log.info("Parsed next word id -> {}", wordForSaveId);

        recomendationService.saveWord(userId, wordForSaveId);



        var updatedMenu = wordMenu.apply(
                wordForSaveId,
                WordMenu.WordMenuState.SAVED
        );
